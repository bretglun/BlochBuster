...
# Define arrow colors
color = [(0,176/256,80/256),'red','cadetblue','chocolate','darkolivegreen','darkslateblue','green','lightslategray','blue','brown']
...
def plotFrame3D(names,comps,title,clock,frame,spoilTextAlpha,RFTextAlpha,RFText):
	...
	ax.text(0,0,1.4, title, fontsize=14, horizontalalignment='center') # Draw title
	time_text = ax.text(-1,-.8,-1, 'time = %.1f msec'%(clock[frame])) # Draw time	
	...
	for c in range(len(comps)):		
		for m in range(nVecs):
			...
			if m==nVecs//2: # Just for getting labels
				ax.plot([0,0],[0,0],[0,0], '-', lw=2, color=col, alpha=1., label=names[c])
			ax.add_artist(Arrow3D([0,M[0,frame]],[0,M[1,frame]],[0,M[2,frame]], mutation_scale=20, arrowstyle="-|>", lw=2, color=col,alpha=alpha,zorder=order))
		

	# Draw "spoiler" and "FA-pulse" text
	ax.text(.7,.7,.8, 'spoiler', fontsize=14, alpha=spoilTextAlpha[frame], color='#500000', horizontalalignment='right')
	ax.text(.7,.7,.95, RFText[frame], fontsize=14, alpha=RFTextAlpha[frame], color='#005000', horizontalalignment='right')
	
		# Draw legend
	handles, labels = ax.get_legend_handles_labels()
	leg = ax.legend([plt.Line2D((0,1),(0,0), lw=2,color=color[(c)%len(color)]) for c,handle in enumerate(handles)],labels,
	loc=2,bbox_to_anchor=[.14, .83])
...
def plotFrameMT(names,comps,title,clock,frame,plotType):
	if plotType not in ['xy','z']: raise Exception('plotType must be xy (for transversal) or z (for longitudinal)')
	
	# create diagram
	xmin,xmax=0,clock[-1]
	if plotType=='xy': ymin,ymax=0,1
	elif plotType=='z': ymin,ymax=-1,1
	fig = plt.figure(figsize=(5,2.7))
	ax = fig.gca(xlim=(xmin,xmax), ylim=(ymin,ymax))
	for side in ['bottom','right','top','left']: ax.spines[side].set_visible(False) # remove default axes
	ax.grid()
	plt.title(title)
	...
	if plotType=='xy':
		...
		for c in range(len(comps)):
			...
			Msum+=Mxy
		if 'water' in names and 'fat' in names: # Special case: also plot sum of fat and water
...
def applyPulseSeq(Meq,w,T1,T2,pulseSeq,w1,Nreps=1,dt=0.1,instantRF=False):
	M = np.array([[0.],[0.],[Meq]]) # Initial state is equilibrium magnetization
	for rep in range(Nreps):
		for (FA,T,spoiler) in pulseSeq:
			...
			if instantRF: M1 = integrate.odeint(derivs,M[:,-1],t,args=(Meq,0.,w1_adj,np.inf,np.inf))
			else: M1 = integrate.odeint(derivs,M[:,-1],t,args=(Meq,w,w1_adj,T1,T2))
			M = np.concatenate((M,M1[1:].transpose()),axis=1)
			# Then relaxation
			if instantRF: t = np.arange(0, T+dt, dt)
			else: t = np.arange(0, T-dur+dt, dt)
			M1 = integrate.odeint(derivs,M[:,-1],t,args=(Meq,w,0.,T1,T2))
			M = np.concatenate((M,M1[1:].transpose()),axis=1)
			# Then spoiling
			if spoiler: M[:,-1]=spoil(M[:,-1])
	return M
...
def getClockSpoilAndRFText(pulseSeq,Nreps,w1,dt,instantRF=False):
	...
	for rep in range(Nreps):
		for (FA,T,spoiler) in pulseSeq:
			...
			if instantRF: clock.extend(np.full(t.shape,clock[-1])) # Clock stands still during instant RF pulse 
			else: clock.extend(t+clock[-1])
			...
			if instantRF: t = np.arange(dt,T+dt,dt)
			else: t = np.arange(dt,T-dur+dt,dt)
	...
	if os.path.isfile(configFile):
		for row in csv.reader(open(configFile, 'rt'), delimiter=':'):
			if row and not row[0].startswith('#'):
				if row[0]=='Title':	title = row[1].strip()
				elif row[0]=='PulseSeq': pulseSeq = readPulseSeq(row[1])
				elif row[0]=='Nreps': Nreps = int(row[1])
				elif row[0]=='CompProps': names,compProps = readCompProps(row[1])
				elif row[0]=='B0': B0 = float(row[1])
				elif row[0]=='B1': B1 = float(row[1])
				elif row[0]=='Nisochromats':	Nisochromats = int(row[1])
				elif row[0]=='IsochromatStep': isochromatStep = float(row[1])
				elif row[0]=='Speed': speed = float(row[1])
				elif row[0]=='Outfile3D': outfile3D = row[1].strip()
				elif row[0]=='OutfileMxy': outfileMxy = row[1].strip()
				elif row[0]=='OutfileMz': outfileMz = row[1].strip()
...
def BlochBuster(configFile, leapFactor=1):
	...
	if not os.path.isfile(convert): raise Exception('ImageMagick:s convert.exe is required but was not found')
	...
	if instantRF: B1=1/(gyro*dt*72)	# Set duration of a 360-pulse to 72 frames		
	w0 = 2*np.pi*gyro*B0	# Larmor frequency [kRad]
	w1 = 2*np.pi*gyro*B1	# B1 rotation frequency [kRad]
	
	# Simulate
	comps = []
	for compProp in compProps: comps.append(simulateComponent(compProp,w0,Nisochromats,isochromatStep,pulseSeq,w1,Nreps,dt,instantRF))
	# Animate
	...
	if not outfile3D+outfileMxy+outfileMz: raise Exception('No outfile (Outfile3D/OutfileMxy/OutfileMz) was found in config')
	tmpdir = r'./tmp'
	outdir = r'./out'
	if os.path.isdir(tmpdir): 
		rmTmpDir = input(r'Temporary folder "{}" already exists. Delete(Y/N)?'.format(tmpdir))
		if rmTmpDir.upper()=='Y': shutil.rmtree(tmpdir)
		else: raise Exception('No files written.')
	
	for (plotType,outfile) in [('3D',outfile3D),('xy',outfileMxy),('z',outfileMz)]:
		if outfile:
			os.mkdir(tmpdir)
			for frame in range(0,nFrames,leapFactor): # Use only every leapFactor frame in animation
				if plotType=='3D': plotFrame3D(names,comps,title,clock,frame,spoilTextAlpha,RFTextAlpha,RFText)
				else: plotFrameMT(names,comps,title,clock,frame,plotType)
